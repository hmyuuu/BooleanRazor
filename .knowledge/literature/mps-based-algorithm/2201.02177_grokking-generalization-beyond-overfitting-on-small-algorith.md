...
**Citation:** arXiv.org, vol. abs/2201.02177, 2022
...
G ROKKING : G ENERALIZATION B EYOND OVERFIT-
                                        TING ON S MALL A LGORITHMIC DATASETS
...
arXiv:2201.02177v1 [cs.LG] 6 Jan 2022
                                                     ...
                                                     gorithmically generated datasets. In this setting, questions about data efficiency,
                                                     ...
                                                     ization can happen well past the point of overfitting. We also study generalization as
...
operands (x + y, x ∗ y, x2 + y 2 and x2 + xy + y 2 ). Such operations tend to require less data for
generalization than closely related non-symmetrical counterparts (x − y, x/y, x2 + xy + y 2 + x).
...
Some operations (for example x3 + xy 2 + y (mod 97)) didn’t lead to generalization within the
...
3.4   Q UALITATIVE V ISUALIZATION OF E MBEDDINGS
...
Stéphane d’Ascoli, Levent Sagun, and Giulio Biroli. Triple descent and the two kinds of overfitting:
...
Jason Weston, Sumit Chopra, and Antoine Bordes.              Memory networks.        arXiv preprint
  arXiv:1410.3916, 2014.
                                                  ...
                                                  5
Jason Weston, Antoine Bordes, Sumit Chopra, Alexander M Rush, Bart van Merriënboer, Armand
...
The following are the binary operations that we have tried (for a prime number p = 97):
          ...
          x ◦ y = x2 + y 2 (mod p) for 0 ≤ x, y < p
          x ◦ y = x2 + xy + y 2 (mod p) for 0 ≤ x, y < p
          x ◦ y = x2 + xy + y 2 + x (mod p) for 0 ≤ x, y < p
          x ◦ y = x3 + xy (mod p) for 0 ≤ x, y < p
          x ◦ y = x3 + xy 2 + y (mod p) for 0 ≤ x, y < p
          x ◦ y = x · y for x, y ∈ S5
...
Figure 6: Effect on data efficiency of introducing k ∈ [0, 10, 100, 1000, 2000, 3000] outliers (exam-
...
the training data (i.e. predicts c = a ◦ b as an answer even for the outlier equations a, b → c0 with
c0 6= c). On the other extreme it could be that the optimization procedure can find networks that
